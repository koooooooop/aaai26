...
class Trainer:
    ...
    def evaluate(self, epoch):
        ...
        with torch.no_grad():
            for x, y, z, _ in self.val_loader:
                ...
                # Note: Inverse transform should be handled carefully if scaler expects 2D array
                # scaler = self.data_module.scaler
                # ...
                
                self.evaluator.update(y, predictions)
    ...
    def run(self):
        for epoch in range(self.config['training']['epochs']):
            # Update labels before starting the training epoch (except for the first one)
